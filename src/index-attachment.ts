/**
 * Seedance 2.0 视频生成 — 附件版入口文件
 * resultType 为 FieldType.Attachment，生成的视频直接存放在表格附件列中
 */
import { basekit, FieldType, field, FieldComponent, FieldCode } from '@lark-opdev/block-basekit-server-api';
const { t } = field;

import { MODEL_OPTIONS, RESOLUTION_OPTIONS, RATIO_OPTIONS, DURATION_OPTIONS, WATERMARK_OPTIONS } from './constants';
  ...
  i18n: {
    messages: {
      'zh-CN': {
        ...
        'field_desc': '调用 Seedance 2.0 API 生成视频，视频直接存放在此列',
      ...
      'en-US': {
        ...
        'field_desc': 'Generate video via Seedance 2.0 API, video saved as attachment',
        'api_key_label': 'API Key',
  ...
  execute: async (params: { [key: string]: any }, context: any) => {
    ...
    try {
      ...
      if (result.status === 'succeeded') {
        ...
        // 返回视频 URL 字符串，飞书会自动将其作为附件存放在表格中
